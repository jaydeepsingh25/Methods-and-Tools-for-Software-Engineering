...
void constructEdges(string inputEdges, vector<vector<bool>> &matrix, vector<tuple<int, int>> &edgesList)
{
    ...
    for (int i = 0; i < match.length(); i++)
    {
      ...
      if (edge.length() > 0)
      {
             ...
             (edge_array[0] != edge_array[1])))
        {
          ...
          edgesList.push_back(make_tuple(edge_array[0], edge_array[1]));
...
vector<int> calculateVertexCover(vector<vector<bool>> &matrix, vector<tuple<int, int>> &edgesList, int kVal)
{
  //cout << "hello" <<endl;
  //cout << kVal << endl;
  ...
  // // version 1 -- possibly wrong
  // for (int i = 0; i < num_vertices; i++)
  // {
  //   Minisat::vec<Minisat::Lit> row_entries;
  //   for (int j=0; j< kVal ; j++){
  //     row_entries.push(~x[i][j]);
  //   }
  //   solver->addClause(row_entries);
  //   row_entries.clear();
  //   //solver->addClause(~x[i][0], ~x[i][1]);
  // }
  ...
  while (!cin.eof())
  {
    ...
    case 'E':
    {
      ...
      else
      {
        ...
        edgesList.clear();
        constructEdges(edges, matrix, edgesList);
        if(edgesList.size() == 0){
          cout << endl;
          continue;
        }
        ...
        {
          int mid = ((low + high) / 2);
          result = calculateVertexCover(matrix, edgesList, mid);
          if(result != temp)
          {
            high = mid - 1;
