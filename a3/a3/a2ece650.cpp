...
void constructEdges(string inputEdges, vector<vector<bool>> &matrix)
{
    ...
    for (sregex_iterator iterator = sregex_iterator(inputEdges.begin(), inputEdges.end(), r);
         iterator != sregex_iterator(); iterator++)
    {
        ...
        for (int i = 0; i < match.length(); i++)
        {
            ...
            /**
             * Create an entry from i to j and j to i in the adjacency matrix to denote edge
            */
            if ((edge_array[0] < (int)matrix.size() && edge_array[1] < (int)matrix.size()) && edge_array[0] != edge_array[1])
            {
    ...
    if (invalidVertex)
    {
        cerr << "Error: Invalid Edge specified" << endl;
...
// computeShortestPath calculates shortest path between startNode and endNode using BFS
void computeShortestPath(int startNode, int endNode, vector<vector<bool>> &matrix)
{
    int numVertices = (int)matrix.size();
    if (startNode < numVertices && endNode < numVertices)
    {
        ...
        // array to keep track of previous node, prev[i] indicates the parent node of node i
        ...
        // now, prev array can be used to construct the path from startNode to endNode
        ...
        if (path[0] == startNode)
        {
            copy(path.begin(), path.end() - 1, ostream_iterator<int>(outputStream, "-"));
            outputStream << path.back();
            cout << outputStream.str() << endl;
        }
        else
        {
            cerr << "Error: No path exists between vertex " << startNode << " and " << endNode << endl;
        }
    }
    else
    {
        cerr << "Error: Invalid vertex specified" << endl;
    }
};
...
int main(int argc, char **argv)
{
    ...
    while (!cin.eof())
    {
        ...
        case 'E':
            if (inputArgs.size() != 2)
            {
                cerr << "Error: Invalid arguments for E command" << endl;
            }
            else
            {
        ...
        case 's':
            if (inputArgs.size() != 3)
            {
                cerr << "Error: Invalid arguments for s command" << endl;
            }
            else
            {
                computeShortestPath(stoi(inputArgs.at(1)), stoi(inputArgs.at(2)), matrix);
            }
            break;
        // case 'p':
        //     for (int i = 0; i < (int)matrix.size(); i++)
